...
class Dumper_Decorator
{
    ...
    {
        $this->sxd = new Sypex_Dumper;
        $this->host = $host;
        $this->port = $port;
        $this->user = $user;
        $this->pass = $pass;

        register_shutdown_function(array($this, 'sxdShutdown'));
    }

    /**
     * @param string $dir
     * 
     * @return Dumper_Decorator
     */
     ...
     * @return Dumper_Decorator
     */
    public function setOutputFile($outFile)
    {
     ...
     * @return bool
     */
    public function backup($db, array $tables = NULL, $charset = 'utf8', $zip = 0, $forceNew = false)
    {
        // Prepare backup job
        ...
        return rename($this->sxd->JOB['file_name'], $this->tmpDir.$this->outputFile);
    }
     ...
     * @return bool
     */
    public function restore($db, $inputFile, $strategy = 0, $zip = 0, $forceNew = false)
    {
        // Prepare restore job
        
        if( !file_exists($inputFile) )
        {
            throw new InvalidArgumentException(sprintf('File "%s" does not exist', $inputFile));
        }
        
        rename($inputFile, $this->restTmpFile = $this->tmpDir . 'sxd_rest_dump');

        switch($zip)
        {
            case 0:
                rename($this->restTmpFile, $this->restTmpFile . '.sql');
                $this->restTmpFile .= '.sql';
                break;
                
            case 10:
                rename($this->restTmpFile, $this->restTmpFile . '.sql.bz2');
                $this->restTmpFile .= '.sql.bz2';
                break;
                
            default:
                rename($this->restTmpFile, $this->restTmpFile . '.sql.gz');
                $this->restTmpFile .= '.sql.gz';
                break;
        }
        ...
        $this->jobData = array (
          ...
          'db' => $db,
          'charset' => '0',
          'file' => basename($this->restTmpFile),
          'strategy' => $strategy,
          'correct' => '0',
          'autoinc' => '0',
          'obj' => 
        ...
        return rename($this->restTmpFile, $inputFile);
    }
    
    protected function run($forceNew)
    {
        set_error_handler(array($this, 'sxdErrorHandler'));
        
        // Resume job?
        if( ! $forceNew && file_exists($this->tmpDir . $this->jobData['job'] . '.job.php') )
        {
            $this->jobData['type'] = 'resume';
        }        
        
        // Create job file
        $this->jobFile = tempnam($this->tmpDir, 'sxd');  
        file_put_contents($this->jobFile, "<?php\n\$JOB = " . var_export($this->jobData, true) . ";", LOCK_EX);
        
        $this->sxd->init(array("-h={$this->host}", "-u={$this->user}", "-p={$this->pass}", "-o={$this->port}", "-j={$this->jobFile}"), $this->tmpDir); 
        
        restore_error_handler();
    }
    
    public function sxdErrorHandler($errno, $errmsg, $filename, $linenum, $vars)
    {
        ...
        if($errno == 8 || $errno == 1024) {
            if (!$this->sxd->fh_log && !$this->sxd->fh_rtl) echo isset($_POST['ajax']) ? "alert('" . mysql_escape_string($str) . "');" : $str;
            else {
                fwrite($this->sxd->fh_log, date('Y.m.d H:i:s') . "\t3\t{$str}\n");
            }
        }
        elseif($errno < 1024) {
            $this->sxd->error = true;
            if (!$this->sxd->fh_log && !$this->sxd->fh_rtl) echo isset($_POST['ajax']) ? "alert('" . mysql_escape_string($str) . "');" : $str;
                ...
                $this->sxd->rtl[1] = time();
                $this->sxd->rtl[9] = 5;
                fseek($this->sxd->fh_rtl, 0);
                fwrite($this->sxd->fh_rtl, implode("\t", $this->sxd->rtl));
                fwrite($this->sxd->fh_log, date('Y.m.d H:i:s') . "\t4\t{$str}\n");
                unset($this->sxd->rtl);
            }
    ...
    public function sxdShutdown()
    {
        if(isset($this->sxd->fh_rtl) && is_resource($this->sxd->fh_rtl) && !empty($this->sxd->rtl) && empty($this->sxd->error)) {
		$this->sxd->rtl[1] = time();
		if(!empty($this->sxd->JOB['file_stp']) && file_exists(dirname(__FILE__) . '/' . $this->sxd->JOB['file_stp'])){
			$type = file_get_contents(dirname(__FILE__) . '/' . $this->sxd->JOB['file_stp']);
			$this->sxd->rtl[9] = !empty($type) ? $type : 2;
		}
		else $this->sxd->rtl[9] = 5;
		fseek($this->sxd->fh_rtl, 0);
		fwrite($this->sxd->fh_rtl, implode("\t", $this->sxd->rtl));
        }
        
        if(isset($this->sxd->JOB['file']) && file_exists($this->sxd->JOB['file']) )
        {
            unlink($this->sxd->JOB['file']);
        }
    }
}

class Sypex_Dumper {
    public $fh_log, $fh_rtl, $rtl, $try, $tmpFiles = array();
	...
	function init($args = false, $tmpDir = 'backup/'){
        // fixed
...
//		}
		...
		if ($args) { // консольный режим
		    ...
		    if($auth && !empty($this->CFG['sjob'])){
                ...
                // ????
                //if(file_exists($this->JOB['file_tmp'])) unlink($this->JOB['file_tmp']);
		    }
	...
	function loadJob($job){
        ...
        // fixed
        // Если начинается со слеша, то $job --- абсолютный путь до файла с задачей
        if( substr($job, 0, 1) === '/' || preg_match('/^[A-Z]:/i', $job) )
        {
            $file = $job;
        }
        else
...
//		}
	...
	function resumeJob($job){
        ...
        $this->tmpFiles[] = $this->JOB['file_rtl'];
		$this->fh_log = fopen($this->JOB['file_log'], 'ab');
        $this->tmpFiles[] = $this->JOB['file_log'];
		$t = fgets($this->fh_rtl);
		if(!empty($t)){
			$this->rtl = explode("\t", $t);	
		}
        ...
        unlink($this->CFG['backup_path'] . $job['job'] . '.job.php');
	}
...
//		$this->addLog("{$this->LNG['combo_file']} {$this->JOB['file']}");
		$this->runRestoreJob();
        unlink($this->CFG['backup_path'] . $this->JOB['job'] . '.job.php');
	}
                ...
                while (false !== ($file = readdir($dh))) { 
                    if (preg_match("/^{$name}_(\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})\.sql/", $file, $m)) { 
                        if ($deldate && $m[1] < $deldate) {
                            unlink($this->CFG['backup_path'] . $file);
...
//			/*if(!$this->rtl[7])*/ $this->addLog(sprintf($this->LNG['set_names'], $collation));
	...
	function getFileObjects($tree, $name, $formatTree = true){
        ...
        if( $name[0] !== '/' && !preg_match('/^[A-Z]:/i', $name) )
        {
