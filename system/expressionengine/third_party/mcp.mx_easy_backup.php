...
class Mx_easy_backup_mcp
{
	...
	function make_backup($task_id = false, $CP = true)
	{
		...
		$this->target_dir = (trim($task_settings['local_path']) != "") ? $this->EE->functions->remove_double_slashes(trim($task_settings['local_path']) . '/') : ((trim($task_settings['local_path']) != "") ? $task_settings['local_path'] : sys_get_temp_dir());
		...
		if ($bkname = $this->file_backup($this->target_dir, $backup_filename . '.file', $task_settings, $this->settings['method'])) {
			...
			$this->upload_list[$bkname]['filesize']  = filesize($this->EE->functions->remove_double_slashes($this->target_dir . '/' . $bkname));
	...
	function delete_backup($backup_id = false, $redirect = true)
	{
		...
		if ($query->num_rows()) {
			...
			$this->EE->$send_to_funct->delete(array(
				...
				), $this->EE->functions->remove_double_slashes($this->target_dir . '/'));
	...
	function download_func($backup_id, $direct = true)
	{
		...
		if ($query->num_rows()) {
			...
			$this->target_dir = ($query->row()->method == 'none') ? $this->EE->functions->remove_double_slashes($this->settings['local_path'] . '/') : $this->EE->functions->remove_double_slashes($this->settings['local_path'] . '/' . 'temp/');
	...
	function file_backup($path, $bk_filename, $task_settings, $method = 'php', $archive_method = "gzip")
	{
		...
		if (isset($task_settings['config_files'])) {
			$dir_list[] = $this->EE->functions->remove_double_slashes(APPPATH . '/config/');
		...
		if (isset($task_settings['templates_folder'])) {
			$dir_list[] = $this->EE->functions->remove_double_slashes(APPPATH . 'templates/');
		}

		if (isset($task_settings['language_folder'])) {
			$dir_list[] = $this->EE->functions->remove_double_slashes(APPPATH . '/language/');
		}
		...
		if (isset($task_settings['optional_files'])) {
			...
			foreach ($optional_files as $val) {
				if (trim($val) != "") {
					if (is_dir($val)) {
						$dir_list[] = $this->EE->functions->remove_double_slashes($val . '\\');
					} else {
						$file_list[] = $val;
					}
				}
			...
			if (!empty($task_settings['exclude_files_list'])) {
				...
				foreach ($exclude_files as $val) {
					if (trim($val) != "") {
						if (is_dir($val)) {
							$exclude_dirs = $exclude . ' ' . $this->EE->functions->remove_double_slashes($val . '/');
						} else {
		...
		if ($method == 'php') {
			if (!defined('PCLZIP_TEMPORARY_DIR')) {
				define('PCLZIP_TEMPORARY_DIR', $this->EE->functions->remove_double_slashes($path . '/'));
			}
			;

			$archive = new PclZip($this->EE->functions->remove_double_slashes($path . '/' . $bk_filename . '.zip'));
